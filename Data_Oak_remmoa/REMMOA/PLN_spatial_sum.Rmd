...
map_species<-function(species,mat,legend = "Obs count", title="",low="blue", high="firebrick1", filter=0.5, vecBounds=NULL,alpha=1){ 
  ...
  finaltitle=ifelse(is.null(title),species,title)
  effort_plot + geom_point(data = data, aes(x = Xcart, y = Ycart, size = !!species, color = !!species), alpha=alpha)+   scale_size(name = legend)+scale_colour_gradient(name =legend,low=low,high=high)+
    guides(size=FALSE)+ ggtitle(finaltitle)
}
```
...
map_species(species=sumN,mat=commonEffect,title="",legend="Detect.",filter=0, low="steelblue1",high="red",alpha=0.5)

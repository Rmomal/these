...
setwd(WorkDir <- "/Users/raphaellemomal/these/Data/REMMOA")
...
```{r}
X$P_O=ifelse(X$Region.Label%in%c("N_C_O1","N_C_O2","N_C_O3"),"O","P")
ggplot(data = X, aes(x = reorder(SUBJECTIVE, -SEA_STATE), fill = factor(SEA_STATE))) +
...
X %>% ggplot(aes(DistCot,Depth, color=Region.Label))+geom_point()+scale_colour_brewer(palette="Dark2")
...
**A lot of zeros : 1510 empty rows**.
...
Global occurances per date (not transect) : 1 if detected on a day. (in $N$, possibility of several detections per transect).
...
Percentages of non-nul counts accross species :
```{r,echo=FALSE, fig.width=10}
...
ggplot(data, aes(x = reorder(data$species, -v), y = v)) + 
  geom_bar(stat = "identity",fill="dodgerblue4")+
   geom_text(aes(label=round(v,1)), position=position_dodge(width=0.9), vjust=-0.25)+theme_minimal()+
...
Counts vary largely between species.

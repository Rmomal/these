...
\begin{frame}{Spanning trees}
	Spanning trees are another \emphase{sparse} solution :
	$$
...
\includegraphics[width=5cm]{compar_typegraphs.png}
	...
	\end{column}
	 \begin{column}{6cm}
	Much \emphase{ smaller space} to explore:\\ $\text{\#} \mathcal{G} = 2^{\frac{p(p-1)}{2}}$ vs. $\text{\#} \mathcal{T} = p^{(p-2)}$
\end{column}
\end{columns}
...
\[
            \left.
                ...
                Z_i \textit{ iid } &\sim \mathcal{N}_{d}(\mu,\Sigma)\\
                    ...
                    Y_{ij}|Z_{ij} &\sim \mathcal{P}(e^{\only<2-3>{\emphase{x_i^{\text{T}}\Theta_j} +}Z_{ij}}) 
                   
                \end{array}
            \right \} Y \sim \mathcal{PLN}(\only<2-3>{\emphase{x^{\text{T}}\Theta }+}\mu, \Sigma)  
            \]
\end{block}
\vspace{0.5cm}
\begin{itemize}
  ...
  \only<2-3>{ \item Allow adjustment for covariates}
\end{itemize}
%\pause

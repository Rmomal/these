...
\begin{frame}{Graphical models: a statistical framework}
\begin{columns}
...
\item[Maximum spanning tree] Kruskall's algorithm\\
 $$ \hat{T} = \underset{T}{\mathrm{argmax }} \left\{\prod_k q_k (Y)\prod_{(k,l) \in T} q_{k,l}(Y) \right\}  \rightarrow \emphase{\Theta (p^2)}$$ \bigskip 
\item[Tree averaging] Matrix tree theoem \cite{matrixtree}\\ 
$$\mathds{P}(Y) = \sum_T \mathds{P}(T)\mathds{P}(Y|T)  \rightarrow \emphase{\Theta (p^3)}$$
...
\end{itemize}
%\pause
\only<3>{\center{ \textbf{ Approach:} Infer the \emphase{latent Gaussian network} with an \emphase{EM algorithm.}}}
\end{frame}

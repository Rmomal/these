...
library(ape); library(sna); library(poilog);library(tidyverse)

# Parms
p = 10; 
#B.path = 1e6; R.subsample = 1e3
B.path = 1e5; R.subsample = 1e3
...
accept/B
max(diff(which(Y.path[2:B, 1]!=Y.path[1:(B-1), 1]))) # plus long temps de séjour

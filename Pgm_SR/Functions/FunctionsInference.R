...
#########################################################################
FitBetaStatic <- function(beta.init, phi, iterMax = 1e3, eps = 1e-6){
   # beta.init = beta.unif; iterMax = 1e3; eps = 1e-6; log.phi = log(phi)
   beta.tol = 1e-4; beta.min = 1e-30
   beta.old = beta.init / sum(beta.init); log.phi = log(phi)
   iterPrint = round(sqrt(iterMax))
   iter = 0; logpY = rep(0, iterMax); diff = 2*eps
   while ((diff > eps) & (iter < iterMax)){
      iter = iter+1; 
      P = Kirshner(beta.old*phi)$P; # sum(P)
      P = EdgeProba(beta.old*phi); # sum(P)
      beta = F_Vec2Sym(optim(F_Sym2Vec(beta.old), F_NegLikelihood, gr=F_NegGradient, method='BFGS', log.phi, P)$par)
      ...
      # cat(iter, ':', min(P), max(P), '/', sum(beta)/2, '/', logpY[iter], diff, '\n')
      if (iter %% iterPrint==0){cat(iter, ' (', logpY[iter], ', ', diff, ') ', sep='')}
      beta.old = beta
   }
   cat(iter, ' (', logpY[iter], ', ', diff, ')\n', sep='')
   logpY = logpY[1:iter]
   # plot(logpY)

...
source('/Users/raphaellemomal/these/R/codes/FunctionsMatVec.R')
source('~/R/codes/missingActor/LiTree/functions-estimation.R')
...
load("/Users/raphaellemomal/these/R/codes/Spatial/BarentsFish.Rdata")
 Xobs = Data$count; Xmiss=Data$covariates[, 4]
 d=31

# Infer missing edge
init <- initEM(cov(Xobs), cliquelist = findCliques(cov(Xobs), k+1)[1:k],n=n)
...
plot(Xpred, Xmiss , xlim=c(-100, 100), main=round(cor(Xpred, Xmiss), 2)); abline(0, 1)

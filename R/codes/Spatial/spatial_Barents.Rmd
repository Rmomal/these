...
comparVgm_plot<-function(spInd,line=TRUE, cress){
  ...
  #no trend :
  v1=variogram(log(count+0.1)~1, spatialData, cressie=cress)
  ...
  v5=variogram(log(count+0.1)~Latitude+Longitude, spatialData, cressie=cress)
 # browser()
  v1=cbind(v1[,c(2,3)],Model="null")
  # v2=cbind(v2[,c(2,3)],trend="temp+depth")
  # v3=cbind(v3[,c(2,3)],trend="complet")
  # v4=cbind(v4[,c(2,3)],trend="temp")
  v5=cbind(v5[,c(2,3)],Model="spat. \ncoordinates")
  Vtot=rbind(v1,v5)
  
  g=ggplot(Vtot, aes(x=dist,y=gamma,color=Model, shape=Model))+geom_point()+labs(x="Distance",y="Semi variance")+
    theme_minimal()+scale_color_manual(values=c("#00A1D5FF","chartreuse3","indianred2","orange2","gray40"))  
...
g=comparVgm_plot(21, line=TRUE,cress=FALSE)
ggsave("variogram.png", plot=g, width=6, height=3,path= "/Users/raphaellemomal/these/R/images")
```

Estimateur classique du variogramme :
```{r}
collec=which(colSums(Y)%in%sort(colSums(Y),decreasing = TRUE)[1:10] )
sapply(4, function(x) comparVgm_plot(x, line=TRUE,cress=TRUE))
```

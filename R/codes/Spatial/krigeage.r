...
emp_vario_intra <- function(y,
                            ...
                            n_sim = 100,
                            distmat = NULL,
                            region=NULL, region.label,esp,perTransect=FALSE
){
  ...
  if(is.null(coupes)) {
    ...
    coupes <- c(-0.01, 0.001, 1, seq(2.5, 100, 2.5))
  }
  ...
  TauxObs <- function(y, esw, l){
    ...
    distrib=ifelse(overdispersion(y)<1, "poisson", "poisson")
    model_m <- glm(y ~ 1 + offset(2 * esw * l), family = distrib, control = my_control)
  ...
  if(is.null(distmat)) {
    ...
    if(perTransect){
      dl <- as.matrix(sqrt((center[-1, 1] - center[-length(y), 1])^2 + (center[-1, 2] - center[-length(y), 2])^2))
      ...
      distclas <- cut(distmat, breaks = coupes)
    }else{
      convert2km <- ifelse(projected, 1/1e3, (60 * 1.852)) # convertir en km (si WGS84, 1°lat = 60'= 60 * 1.852 km soit environ 110km)
    distmat <- fields::rdist(center,center)* convert2km
   # breaks2=c(-0.01, 0.01,1:4,seq(5,100,5), seq(105, max(distmat),20))
    breaks2=coupes
    distclas <- cut(distmat, breaks = breaks2)
    }
...
# gaussien
###############
emp_vario_intra_gaussien <- function(y,
                                     esw,
                                     l,
                                     center,
                                     projected = TRUE,
  ...
  if(is.null(distmat)) {
    ...
    if(perTransect){
      ...
      distmat <- fields::rdist(center,center)
    }
  ...
  if(plot) {
    ...
    bootstrap_pval <- function(x, simulator, statistic, n_sim, alpha, truth) {
      tboots <- replicate(n_sim, statistic(simulator (x))) 
      ci_lower <- 2*statistic(x) - quantile(tboots, 1 - alpha/2)
      ci_upper <- 2*statistic(x) - quantile(tboots, alpha/2)
      return(as.numeric(ifelse(ci_lower > truth, 0, ifelse(ci_upper < truth, 0, 1))))
    }
    # browser()

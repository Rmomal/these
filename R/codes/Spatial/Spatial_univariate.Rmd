...
effort_plot
ggsave("carte_vierge2.png")
#effort_plot
### visualiser les detections (N) de petits delphinin?s
map_species<-function(species,mat,titre = "Obs count", low="blue", high="firebrick1", filter=0.5, vecBounds=NULL){ #SMADEL
  #  browser()  
  species=enquo(species)
...
map_species(GRETER,N)
map_species(FREGATE,Y)
map_species(BROPET,N)
map_species(FOUS,Y, count=TRUE, vecBounds=c(7.76e6,max(X$Ycart),2e5, 6e5))
map_species(BROPET,N, count=TRUE, vecBounds=c(7.4e6,7.55e6,5.5e5, 9.5e5))

map_species(DASSPP,M)
...
map_species(sum,W,low="lightskyblue",high="blue4", filter=1, titre="Biodiversité")

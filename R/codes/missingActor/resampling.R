...
resamp_nullJcor<-function(sigma_obs,r,B=10,alpha=0.1,eps=1e-3){
  ...
  SO=SO*matsig^2
    ...
    Wginit= init0$Wginit; Winit= init0$Winit; upsinit=init0$upsinit 
    VEM<-tryCatch({VEMtree(null_counts, MO, SO, MH=NULL,upsinit,W_init =Winit,eps=1e-3,
                  Wg_init =Wginit,plot = FALSE, maxIter = 100,print.hist = FALSE,
                  alpha=0.1, verbatim=TRUE, trackJ=FALSE )},error=function(e){e},
                  finally={})
    if(length(VEM)==11){
      J=tail(VEM$lowbound,1)
    }else{J=c(NaN,NaN,NaN,NaN)}
  return(J)
}
...
toc()
tic()
echant_J_nullr0_1<-mclapply(1:100,function(x){
  cat(paste0("\n resampling ",x,"\n")) # 63s 3 cores r=0
  resamp_nullJcor(Sigma_tree,r=0)
}, mc.cores=3)
toc()
echant_J_nullr0_1=data.frame(do.call(rbind, echant_J_nullr0_1))
summary(echant_J_nullr0_0)
...
Jtest_0=tail(VEM0$lowbound,1)
echant_J_nullr0_0 %>% as_tibble() %>% ggplot(aes(J.T1))+
  geom_density(color="cornflowerblue",fill="cornflowerblue", alpha=0.5)+theme_light()+
  geom_vline(xintercept = Jtest_0$T1, color="black", linetype="dashed")
(mean(echant_J_nullr0_0$J.T1, na.rm=TRUE)-Jtest_0$T1)/sd(echant_J_nullr0_0$J.T1, na.rm=TRUE)
median(echant_J_nullr0_0$J.T1, na.rm=TRUE)-Jtest_0$T1

Jtest_1=tail(VEM0$lowbound,1)
echant_J_nullr0_1 %>% as_tibble() %>% ggplot(aes(T1))+
  geom_density(color="cornflowerblue",fill="cornflowerblue", alpha=0.5)+theme_light()+
  geom_vline(xintercept = Jtest_1$T1, color="black", linetype="dashed")
(mean(echant_J_nullr0_1$T1, na.rm=TRUE)-Jtest_1$T1)/sd(echant_J_nullr0_1$T1, na.rm=TRUE)
median(echant_J_nullr0_1$T1, na.rm=TRUE)-Jtest_1$T1

...
Qual<-data.frame(auc=do.call(rbind, lapply(simus, function(seed){
  ...
  return(ppvh)
})), seed = 1:200) %>% as_tibble() %>% mutate(noWconv =(1*(seed %in% weird.conv)), nH = misAct$nH,influence=misAct$influence)
...
Qual %>% group_by(influence) %>% 
  summarise(count=n(), med.auc=round(median(auc), 2),
                              auc.prop80 = round(sum(auc>0.8)*100/count, 1),noWconv.prop=round(sum(noWconv)*100/count, 1))%>% 
  kable() %>% kable_styling()
...
Qual %>% gather(key, value, -nH, -noWconv, -influence, -seed) %>% 
...
data=data.frame(corZM=abs(cor_ZHMH), nH=misAct$nH, influence=misAct$influence) 
data%>% 
  group_by(influence) %>% summarize(med.cor=round(median(corZM),2), prop50 = round(100*sum(corZM>0.5)/n(),1)) %>% kable() %>% kable_styling()

data %>% ggplot(aes(corZM, color=influence, fill=influence))+
...
Pour 40% des graphes, l'arrêt de l'algorithme n'a pas nécessité la convergence de W et la dernière différence sur W observée était supérieure à 0.1. Ceci ne semble pas influer la qualité de l'estimation.
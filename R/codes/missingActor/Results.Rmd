...
simus=readRDS("/Users/raphaellemomal/these/R/codes/missingActor/SimResults/simus_V2_400.rds")
...
datacor%>% 
  group_by(influence) %>% summarize(med.cor=round(median(corUM),2), prop50 = round(100*sum(corUM>0.5)/n(),1)) %>% kable() %>% kable_styling()
...
```{r}
data=data.frame(J,Jcor= Jcor_diff[,1],auc=Qual$auc,ppvh= Qual$ppvh,corZM=datacor$corUM, influence=Qual$influence, max.maxDeg=misAct$max.maxDeg,mean.btw=misAct$mean.btw, btwH=misAct$btwH, tprh=Qual$tprh, mean.maxDeg=misAct$mean.maxDeg, mean.vv=misAct$mean.vv, mean.cpH=misAct$mean.cpH) %>% as_tibble()
```

...
Qual<-data.frame(auc=do.call(rbind, lapply(simus, function(seed){
  ...
  return(ppvh)
})), seed = (1:N)) %>% as_tibble() %>% mutate( nH =misAct$nH,
...
```{r}
which(datacor$influence=="major" & datacor$corZM < 0.5)
```
...
data=data.frame(J,penT, Jcor_diff,auc=Qual$auc,ppvh= Qual$ppvh,tprh=Qual$tprh, corZM=datacor$corZM, influence=Qual$influence) %>% as_tibble()
```
...
```{r}
data=data.frame(J,Jcor= Jcor_diff[,1],auc=Qual$auc,ppvh= Qual$ppvh,corZM=datacor$corZM, influence=Qual$influence, max.maxDeg=misAct$max.maxDeg,mean.btw=misAct$mean.btw, btwH=misAct$btwH, tprh=Qual$tprh, mean.maxDeg=misAct$mean.maxDeg, mean.vv=misAct$mean.vv, mean.cpH=misAct$mean.cpH) %>% as_tibble()
```
...
```{r}
data  %>% 
  ggplot(aes(corZM, btwH, color=influence))+geom_vline(xintercept = 0.5, color="gray")+geom_point()+facet_wrap(~influence, scales="free_y")+mytheme.dark("")
data  %>% 
  ggplot(aes(corZM, mean.btw, color=influence))+geom_vline(xintercept = 0.5, color="gray")+geom_point()+facet_wrap(~influence, scales="free_y")+mytheme.dark("")
data  %>% 
  ggplot(aes(corZM, max.maxDeg, color=influence))+geom_vline(xintercept = 0.5, color="gray")+geom_point()+facet_wrap(~influence, scales="free_y")+mytheme.dark("")

data  %>% 
  ggplot(aes(corZM, mean.vv, color=influence))+geom_vline(xintercept = 0.5, color="gray")+geom_point()+facet_wrap(~influence, scales="free_y")+mytheme.dark("")
data  %>% 
...
data  %>% 
  ggplot(aes(corZM, mean.maxDeg, color=influence))+geom_vline(xintercept = 0.5, color="gray")+geom_point()+facet_wrap(~influence, scales="free_y")+mytheme.dark("")

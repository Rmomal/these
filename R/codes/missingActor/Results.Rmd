...
Qual %>% group_by(influence) %>% summarise( count=n(), med.ppvh=round(median(ppvh, na.rm=TRUE),2), med.tprh=round(median(tprh),2), ppvh.prop80 = round(sum(ppvh>0.8, na.rm=TRUE)*100/count, 1),tprh.prop80 = round(sum(tprh>0.8)*100/count, 1))%>% 
  kable() %>% kable_styling()
...
nb_occ=c(do.call(rbind, lapply(simus, function(graine){
  graine$VEM_1$nbocc
})))
Qual$nb_occ=nb_occ
Qual %>% ggplot(aes(nb_occ>1,auc, color=influence, fill=influence ))+geom_boxplot(alpha=0.3)+
  facet_wrap(~influence)+mytheme.dark("")
```

...
```{r}
# simu parameters
set.seed(1) # mauvais 2, 4, 5 on a 2 lié à plein de gens donc pas trouvé et mauvais precrec H,
...
clique_mclust$init
computeFPN(cliqueList = clique_mclust$init,trueClique = trueClique,p=p)
plotInitMclust(res=clique_mclust,title = "")
init=initVEM(counts = counts,  initviasigma=clique_mclust$init, sigma_obs,MO=MO,r = r)
Wginit= init$Wginit; Winit= init$Winit; omegainit=init$upsinit ; MHinit=init$MHinit
...
run.method<-function(cliques,method,counts,sigma_obs,MO,SO,
  ...
  best=which.max(vBIC)
  data=computeFPN(cliques, trueClique, p=p) %>% 

...
FitSparsePCA <- function(Y, q=1, alphaGrid=10^(seq(-4, 0, by=.1))){
  ...
  n <- nrow(Y); p <- ncow(Y); alphaNb <- length(alphaGrid); 
  # Fits all sparsePCA
  sPCA <- list()
  for(a in 1:alphaNb){
...
boot_FitSparsePCA<-function(Y, B){
  lapply(1:B, function(x){
    ...
    Y.sample=Y[ech,]
    c=FitSparsePCA(Y.sample)$cliques
    return(c)
  })
...
run.method<-function(sigma_obs,cliques,method){
...
SelectInitClique<-function(seed, B, n=200, p=14, r=1){
  ...
  varclust.choice<-run.method(sigma_obs, cliques_varclust, "VarClust")
  #---
  choices<-rbind(spca.choice, mclust.choice , varclust.choice,

...
# vBIC computations
run.VEM<-function(init,counts,sigma_obs,MO,SO,W_init,Wg_init,r){
  p=ncol(counts); O=1:p
...
SelectInitClique<-function(seed, B, n=200, p=14, r=1){
  ...
  matcovar=matrix(1, n,r) ; sigma_obs=PLNfit$model_par$Sigma
  init=initVEM(counts = counts, trueClique = trueClique,initviasigma=trueClique, sigma_obs,r = r)
  Wginit= init$Wginit; Winit= init$Winit; omegainit=init$omegainit ; MHinit=init$MHinit
  ome_init=omega[c(setdiff(1:(p+r), hidden), hidden),c(setdiff(1:(p+r), hidden), hidden)]
  ome=ome_init ; diag(ome)=0
  ...
  vem=run.VEM(init = trueClique[[1]],counts=counts, sigma_obs, MO=MO, SO=SO, 
              W_init=Winit, Wg_init=Wginit, r=r)
  ...
  true.perf<-data.frame(t(sapply(list(vem), function(x){
    ...
    PPVH=accppvtpr(x$Pg,ome,h=(p+1):(p+r),seuil=quantile(x$Pg[,h], 0.75))[5]
  ...
  vem_mclust=lapply(cliques_mclust, function(init){
    run.VEM(init = init,counts=counts,sigma_obs, MO=MO, SO=SO, W_init=Winit, Wg_init=Wginit, r=r)
  })
  ...
  sPCA.Z <- FitSparsePCA(scale(MO))
  cliques_spca=c(sPCA.Y$cliques,sPCA.Z$cliques)
  vem_spca=lapply(cliques_spca, function(init){
    run.VEM(init = init,counts=counts, sigma_obs,MO=MO, SO=SO, W_init=Winit, Wg_init=Wginit, r=r)
  })
  vec.perf<-data.frame(t(sapply(vem_spca, function(x){
    auc=round(auc(pred = x$Pg, label = ome),4)
    PPVH=accppvtpr(x$Pg,ome,h=(p+1):(p+r),seuil=quantile(x$Pg[,h], 0.75))[5]
    return(c(auc=auc, PPVH=PPVH))
  })))
  vBIC_spca<-vec.vBIC(vem_spca,counts,theta, matcovar, r=r)
  spca.choice= computeFPN(cliques_spca, trueClique, p) %>% 
    mutate( vBIC=vBIC_spca, sizes=unlist(lapply(cliques_spca, length)), auc=vec.perf$auc,
            PPVH=vec.perf$PPVH,method=c("spca.Y", "spca.Z"))
  #---
  
  choices<-rbind(spca.choice, mclust.choice , 
                 data.frame(FP=0,FN=0, vBIC=truevBIC,sizes=length(trueClique[[1]]), 

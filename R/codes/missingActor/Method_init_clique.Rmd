...
# Proximity and global performance {.tabset}

FN seems to control all performances.
```{r}
quality=quality %>% mutate(initFN = FN*nH, initTP = nH-trueFN, initFP = (trueFN*FP)/(1-FP),
                   PPVHinit=initTP/(initTP+initFP), TPRHinit = initTP/nH)
```

## PPVH
```{r}

g=quality %>% filter(method!="Oracle", !is.na(PPVH), method=="sPCA") %>% group_by(influence,  PPVHinit,TPRHinit) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% dplyr::select(prop,PPVH,TPRH, PPVHinit, TPRHinit, influence) %>% 
  gather(key, value, -prop, -PPVHinit,-TPRHinit,-influence) %>% 
  ggplot(aes(PPVHinit,TPRHinit,color=value, size=prop))+geom_point() +
  facet_grid(key~influence) +mytheme.bg

g=quality %>% filter(method!="Oracle", !is.na(PPVH), method=="sPCA") %>% group_by(influence,  FN, FP) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% dplyr::select(prop,PPVH,TPRH, FN, FP, influence) %>% 
  gather(key, value, -prop, -FN,-FP,-influence) %>% 
  ggplot(aes(FP,1-FN,color=value, size=prop))+geom_point() +
  facet_grid(key~influence) +mytheme.bg+labs(x="initial FPR", y="initial TPRH")
ggsave(plot=g,filename = "quali_init2.png", path =  "/Users/raphaellemomal/these/R/images",
       width=7, height=5 )
```
...
## TPRH
```{r}
quality %>% filter(method!="Oracle", !is.na(TPRH)) %>% group_by(influence,method,  FP,FN) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence,method ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% 
  ggplot(aes(FP,FN,color=TPRH, size=prop))+geom_point() +
  facet_grid(method~influence) +mytheme.bg
```

## AUC/FP
```{r}
quality %>% filter(method!="Oracle", !is.na(auc)) %>% group_by(influence,method,  FP,FN) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence,method ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% 
  ggplot(aes(FP,FN,color=auc, size=prop))+geom_point() +
  facet_grid(method~influence) +mytheme.bg

```

# Proximity and performance of best FN {.tabset}

We filter all seeds by their minimum FN reached, for each method and influence.

##PPVH
```{r}
quality %>% filter(method!="Oracle", !is.na(PPVH)) %>% group_by(influence,method ,seed) %>%
  filter(FN==min(FN)) %>% 
  ungroup()%>% group_by(influence,method,  FP,FN) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence,method ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% 
  ggplot(aes(FP,FN,color=PPVH, size=prop))+geom_point() +
  facet_grid(method~influence) +mytheme.bg
```

##TPRH
```{r}
quality %>% filter(method!="Oracle", !is.na(TPRH)) %>% group_by(influence,method ,seed) %>%
  filter(FN==min(FN)) %>% 
  ungroup()%>% group_by(influence,method,  FP,FN) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence,method ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% 
  ggplot(aes(FP,FN,color=TPRH, size=prop))+geom_point() +
...
##AUC
```{r}
quality %>% filter(method!="Oracle", !is.na(auc)) %>% group_by(influence,method ,seed) %>%
  filter(FN==min(FN)) %>% 
  ungroup()%>% group_by(influence,method,  FP,FN) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence,method ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% 
  ggplot(aes(FP,FN,color=auc, size=prop))+geom_point() +
...
# Proximity and performance of best J {.tabset}
...
```{r}
quality %>% filter(method!="Oracle") %>% group_by(influence,method ,seed) %>%
  filter(is.na(J)) %>% 
  ungroup()%>% group_by(influence,method,  FP,FN) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence,method ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% 
  ggplot(aes(FP,FN, size=prop))+geom_point() +
  facet_grid(method~influence) +mytheme.bg
```
...
```{r}
quality %>% filter(method!="Oracle") %>%  group_by(seed,influence, method) %>% 
  summarize(moy=mean(FP),maxJ=FP[which(J==max(J, na.rm=TRUE))],min=min(FP), fail=mean(FP[which(is.na(J))]), Jna=mean(is.na(J))) %>%
  dplyr::select(-Jna) %>% gather(key, FP,-influence, -method,-seed) %>% 
  ggplot(aes(x=influence,y=FP, color=key, fill=key))+geom_boxplot(alpha=0.3)+facet_wrap(~method)+mytheme.dark("")


quality %>% filter(method!="Oracle") %>%  group_by(seed,influence, method) %>% 
  summarize(moy=mean(FN),maxJ=FN[which(J==max(J, na.rm=TRUE))],min=min(FN), fail=mean(FN[which(is.na(J))]), Jna=mean(is.na(J))) %>%
  dplyr::select(-Jna) %>% gather(key, FN,-influence, -method,-seed) %>% 
  ggplot(aes(x=influence,y=FN, color=key, fill=key))+geom_boxplot(alpha=0.3)+facet_wrap(~method)+mytheme.dark("")
```

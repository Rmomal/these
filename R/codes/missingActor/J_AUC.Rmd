...
```{r,echo=FALSE}
seed_filtre %>%filter(goodDelta) %>%   group_by(seed, filtre) %>% mutate(maxJ = max(J)) %>%   filter(J==maxJ) %>% dplyr::select(seed, filtre,maxJ, AUC, ppvh) %>% kable(escape=FALSE, format="html",caption="Sélection avec J")%>% 
  collapse_rows() %>% kable_styling()
```

```{r,echo=FALSE}
seed_filtre %>%filter(goodDelta) %>%   group_by(seed, filtre) %>% mutate(maxJcor = max(Jcor)) %>%   filter(Jcor==maxJcor) %>% dplyr::select(seed, filtre,maxJcor, AUC, ppvh)  %>% kable(escape=FALSE,format="html",caption="Sélection avec J corrigée") %>% 
  collapse_rows()%>% kable_styling()
```
### AUC
```{r}

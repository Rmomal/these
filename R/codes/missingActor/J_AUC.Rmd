...
seed_filtre %>%  group_by(seed, filtre) %>% mutate(maxJ = max(J)) %>% 
  filter(J==maxJ) %>% dplyr::select(maxJ, AUC, ppvh) %>% kable() %>% kable_styling()
```
...
seed_filtre %>% group_by(seed) %>% summarise(JPLN_SigT = JPLN_SigT[1]) %>% kable() %>% kable_styling()
```
...
```{r}
seed_filtre %>% ggplot(aes( diffJPLN,Delta, color=(AUC>0.5)))+geom_point()+
  facet_grid(filtre~seed)+mytheme.dark("AUC>0.5")
...
```{r}
seed_filtre %>%filter(gooddiff) %>%   group_by(seed, filtre) %>% mutate(maxJ = max(J)) %>%   filter(J==maxJ) %>% dplyr::select(maxJ, AUC, ppvh) %>% kable() %>% kable_styling()
```

```{r}
seed_filtre %>%filter(gooddiff) %>%   group_by(seed, filtre) %>% mutate(maxJcor = max(Jcor)) %>%   filter(Jcor==maxJcor) %>% dplyr::select(maxJcor, AUC, ppvh) %>% kable() %>% kable_styling()
```
...
```{r}
seed_filtre %>%filter(goodDelta) %>%   group_by(seed, filtre) %>% mutate(maxJ = max(J)) %>%   filter(J==maxJ) %>% dplyr::select(maxJ, AUC, ppvh) %>% kable() %>% kable_styling()
```

```{r}
seed_filtre %>%filter(goodDelta) %>%   group_by(seed, filtre) %>% mutate(maxJcor = max(Jcor)) %>%   filter(Jcor==maxJcor) %>% dplyr::select(maxJcor, AUC, ppvh) %>% kable() %>% kable_styling()
```
### AUC
```{r}

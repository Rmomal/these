...
```{r}
PLNfit = PLN(counts~X$Temperature)
theta=PLNfit$model_par$Theta
int=matrix(1, nrow(counts),1)
matcovar=cbind(int, X$Temperature)
...
J1<-True_lowBound(counts,BarentsVEM$M,BarentsVEM$S, theta, matcovar,BarentsVEM$W, BarentsVEM$Wg, BarentsVEM$Pg, BarentsVEM$omega )
...
# Position of missing actor in the network

Here we want to characterize the position of the missing actor in the network: what neighbors does it have ? Especially, are they more lined to temperature than other species ?


Maybe not the same scale for observed edges probabilities and others ?

```{r}
hist(BarentsVEM$Pg, breaks=30)
hist(BarentsVEM$Pg[31,], breaks=30)
```


```{r}

netO=1*(BarentsVEM$Pg[1:30,1:30]>0.1)
netH<-1*(BarentsVEM$Pg[31,]>0.06)
network<-cbind(rbind(netO, netH[-31]), c(netH))
draw_network(network, curv=0, layout="kk", pal = "royalblue3")
```

```{r}
R2H<-apply(BarentsVEM$M[,which(netH==1)],2,function(y){
  fit=lm(y~X$Temperature)
  return(signif(summary(fit)$adj.r.squared, 5))
})
corelH<-apply(BarentsVEM$M[,which(netH==1)],2,function(y){
  corel=cor(y,X$Temperature)
})
R2O<-apply(BarentsVEM$M[,-c(which(netH==1), p+1)],2,function(y){
  fit=lm(y~X$Temperature)
  return(signif(summary(fit)$adj.r.squared, 5))
})
corelO<-apply(BarentsVEM$M[,-c(which(netH==1), p+1)],2,function(y){
  corel=cor(y,X$Temperature)
})

dataH<-data.frame(R2=R2H, corel=corelH, type="Neighbors")
dataO<-data.frame(R2=R2O,corel=corelO, type="Others")

R2plot<-rbind(dataH,dataO) %>% gather(key,value, -type) %>% 
   ggplot(aes(value, fill=type, color=type))+geom_density(alpha=0.5)+facet_wrap(~key, scales="free")+ mytheme.dark+labs(x="",y="")
ggsave("R2_missing.png", plot=R2plot, width=7.5, height=4,path= "/Users/raphaellemomal/these/R/images")

...
head(quality) %>% kable() %>% kable_styling()
quality %>% filter(FP==1)
```
...
g=quality %>% filter(method!="Oracle", !is.na(PPVH)) %>% group_by(influence,  FN, FP) %>%  mutate(n=n()) %>% ungroup() %>% group_by(influence ) %>% 
  mutate(ntot=n(),prop=n/ntot) %>% dplyr::select(prop,FPRH,FNRH, FN, FP, influence) %>% 
  mutate(specificity=1-FPRH, sensitivity=1-FNRH) %>% 
  rename(`spec-f`=specificity, `sens-f`=sensitivity) %>% 
  gather(key, value, -prop, -FN,-FP,-influence,-FPRH,-FNRH) %>% 
  ggplot(aes(1-FP,1-FN,color=value, size=prop))+geom_point() +
  facet_grid(key~influence) +mytheme.viridis("Final \nmeasures:\n")+labs(x="specificity",y="sensitivity")#+labs(x="FPR -i", y="FNR -i")+guides(size=FALSE)
g

...
graph.p<-function(p){
  param<-data.frame(sapply(seq(1 / p, 8 / p, 0.05), function(y)
    sapply(r, function(x)
      varyd(d = y, x))))
  colnames(param)<-seq(1 / p, 8 / p, 0.05)
  param$par<-rep(c("r","alpha","beta"),length(r))  
  for (par in c("r","alpha","beta")){
    param_r<-(param[param$par==par,]) %>% 
      gather(d,val,-par)
    param_r$val<-as.numeric(as.character(unlist(param_r$val)))
    param_r$d<-as.numeric(as.character(param_r$d))
    param_r$r<-1:50
    title<-ifelse(par=="r","effective r",par)
    assign(paste0("graph_",par),ggplot(param_r,aes(d,val,color=r))+
             geom_point()+
             labs(y=title))+
      theme(legend.position="none")
  }
  grid_arrange_shared_legend(graph_alpha,
                           graph_beta,graph_r,nrow=3,ncol=1,title=paste0("nb nodes: ",p),position="right")
}

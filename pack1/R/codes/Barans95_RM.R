...
if(Tree.res){
  ...
  invisible(sapply(1:4, function(m){
    Stab.sel5[[m]] <<- F_ResampleTreePLN(Data$count, X[[m]], matrix(0, n, p), B=B.resample, maxIter=5,
                                         cond.tol=1e-8)
  }))
...
load(paste0(data.dir, data.name, '-StabSel5.Rdata'))

Fatlist=list(Stab.sel,Stab.sel1,Stab.sel5)
names(Fatlist)<-c("cv","1","5")
...
build_freq<-function(list){
  ...
  lapply(seq_along(list), function(j){
    ...
    lapply(seq_along(stabsel), function(i) { 
      ...
      mat<-stabsel[[i]][["Pmat"]] # mat B*nb(edges)
...
DiffFreq<-Fatfreq %>% 
  spread(depth,freq) %>% 
  mutate(diff1=cv-`1`,diff5=cv-`5`, diffstart=`1`-`5`) %>% 
  gather(compare, diffs, -model,-index_edge,-cv,-`1`,-`5`) 
...
DiffFreq %>% 
  ...
  theme_minimal()+
  geom_abline()+
  geom_hline(yintercept = freq.sel.thres, size=0.8, linetype="dashed")+
  geom_vline(xintercept = freq.sel.thres, size=0.8, linetype="dashed")+
  scale_color_nejm(name="Iterations:")+
  facet_wrap(~model)+
  labs(x="Selection frequences after convergence",y="Selection frequences")+
  theme(strip.text = element_text(size=11))
ggsave("variation_freqSel.png")

...
&=\underbrace{\prod_{i=1}^n \mathds{P}(y_i|T)}_{\text{A}}\prod_{k,l \in T}\psi_{kl}(Y)
\end{align*}
...
Remembering that we work with standard normal distributions, the last two expressions are greatly simplified :

\[\log(A) = \sum_{i=1}^n-\frac{1}{2}\left(\log(2\pi)-y_i^2\right)\]
\[\log(\psi_{kl}(Y))=\frac{-1}{2}\left(\log\left(1-\rho_{kl}\right)+\frac{\rho_{kl}}
{1-\rho_{kl}}(y_k-y_l)^2\right)\]

...
\[\argmax_{\beta_{kl}} \left\{\sum_{k,l} \tau_{kl}\times \left[ \log(\beta_{kl}) + \log(\psi_{kl}(Y)) \right]
 ...
 We notice that if we multiply the last part of the right term, we obtain the probability for the edge $\{k,l\}$ to be in the tree $T$.
 We then have :
...
\begin{align*}
B^{h+1} &= \sum_{T \in \mathcal{T}} \prod_{k,l\in T} \beta_{kl}^{h+1}\\
 &=\sum_{\{k,l\} \in T } \prod_{u,v\in T} \beta_{uv}^{h+1}+\sum_{\{k,l\} \notin T } \prod_{u,v\in T} \beta_{uv}^{h+1}\\
 &=\lambda^h S_{kl}^h + S_{\backslash kl}^{h+1}
\end{align*}
...
\begin{align*}
  \left.
  \begin{array}{r@{}l}
    B^{h+1} & =|\Delta^{\{u,v\}}(W_{\beta})| \\
    S_{\backslash kl}^{h+1}& =|\Delta^{\{u,v\}}(W_{\beta_{\backslash kl}})|
  \end{array}
  \right\rbrace 
  \lambda^h =\frac{|\Delta^{\{u,v\}}(W_{\beta})| -|\Delta^{\{u,v\}}(W_{\beta_{\backslash kl}})|}{S_{kl}^h}
\end{align*}
\end{document}

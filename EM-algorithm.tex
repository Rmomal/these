...
&=\prod_{j=1}^d \mathds{P}(y_i^j|T)\prod_{(k,l)\in T} \frac{\mathds{P}(y_i^k,y_i^l|T)}{\mathds{P}(y_i^k|T)\times \mathds{P}(y_i^l|T)}\\
...
= \sum_{k,l}  \mathds{P}((k,l)\in T | Y) \times \left[ \log(\beta_{kl}) + \log(\psi_{kl}(Y)) \right]
 ...
 \begin{align*}
 \sum_{T\in \mathcal{T} : (k,l)\in T}\mathds{P}( T | Y)  &= \frac{\sum_{(k,l)\in T} \mathds{P}(T)\mathds{P}(Y|T)}{\sum_{T} \mathds{P}(T)\mathds{P}(Y|T)}\\
 &=\frac{\sum_{(k,l)\in T} \prod_{uv} \beta_{uv} \prod_{i} \psi_{uv}(Y_i)}{\sum_{T} \prod_{uv} \beta_{uv} \prod_{i} \psi_{uv}(Y_i)}
 \end{align*}

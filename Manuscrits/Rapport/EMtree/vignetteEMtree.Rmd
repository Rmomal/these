...
The `EMtree` package provides with simulation functions for graphs as well as for count data under the Poisson log-Normal model. Four types of graphs are available in the `generator_graph()` function: erdos (Erdös-Reyni), cluster, scale-free (from the `huge` package) and spanning tree (from the `vegan` package).
```{r}
p=20

...
\NormalTok{data=}\KeywordTok{generate_missing_data}\NormalTok{(n, p, r,}\DataTypeTok{type=}\StringTok{"scale-free"}\NormalTok{, }\DataTypeTok{plot=}\OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{sigma_obs=PLNfit}\OperatorTok{\$}\NormalTok{sigma_O}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.65\linewidth]{nestorArticle/man/figures/README-unnamed-chunk-9-1} \end{center}
...
\DataTypeTok{eps=}\FloatTok{1e-3}\NormalTok{,} \DataTypeTok{maxIter =} \DecValTok{50}\NormalTok{, }\DataTypeTok{alpha=}\FloatTok{0.1}\NormalTok{, }\DataTypeTok{cores=}\FloatTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Wrong fits can be identified by their small ouput size:
...
Finally, as this is an example on simulated data we can assess the performance of each converged fit with their

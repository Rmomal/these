...
\log|\Omega_T|  &= \sum_i \log(\omega_{Tjj}) + \sum_{jk \in T}\log\left( 1- \frac{\omega_{Tjk} ^2}{\omega_{Tjj}\omega_{Tkk}}\right)
\end{empheq}

\begin{itemize}
\item \textbf{vérifier les correspondances avec $\tilde{\Sigma}$ cf sa définition en fin de doc}
\item \textbf{compter une seule fois deux fois les arêtes, sauf pour le déterminant qui est défini par lauritzen}
\item \textbf{dérivée cookbook $\Omega$ matriciel pour vérifier}
\end{itemize}
...
\end{empheq}
\begin{align*}
\Esp\left[-\frac{1}{2}\sum_i Z^T \Omega_T Z\,\middle\vert\,  Y,T\right]&= -\sum_i \left( \sum_{jk \in T} \omega_{Tjk}m_{Tij}m_{Tik} + \sum_{jh\in T} \omega_{Tjh}^2 \omega_{Th}^{-1}(m_{Tij}^2 + S_{Tij})\right)\\
& \;\;\;\; -\frac{1}{2} \sum_i \left(\sum_j \omega_{Tjj}S_{Tjj} + \sum_h\omega_{Th}\left( \omega_{Th}^{-1} + \omega_{Th}^{-2}\Omega_{TOh}^T\tilde{\Sigma}_O\Omega_{TOh}\right)\right)\\
\end{align*}


First solution: use the estimate of Z's covariance matrix given by PLNmodels:
$$\tilde{\Sigma}_{jj} = \frac{1}{n}(M^TM + \sum_i diag(S_i)) $$

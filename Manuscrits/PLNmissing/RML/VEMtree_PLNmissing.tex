...
\end{align*}
The "VE" step of PLNmodels algorithm maximizes $\mathcal{J}_{PLN}$ and gives estimates of $Z_O$ moments of order 1 and 2. So we have easy access to $\widetilde{m}_O, \widetilde{S}_O, \widetilde{\overline{\Omega_{Tm}}}$.  \\
...
The VE step minimizes the KL divergence:
...
Under distribution $h$, $Z_H$ and $Z_O$ are independent. The distribution $h_O$ is already computed, therefore we obtain:
\begin{align*}
\argmin_{g,h(Z_H)} KL\left(g(T)h(Z) || p(Z,T|Y)\right) &=\argmin_{g,h(Z_H)} \{ \Esp_g[\log g(T)] + \Esp_h[\log h(Z_H)] - \Esp_g[\log p(T)]  \\
...
Let's denote $P_g = [\mathds{P}_g(kl \in T)]_{kl}$ the edge probability matrix under the $g$ distribution. We obtain the following derivatives:
\begin{align*}
\frac{\partial}{\partial M_H}&= - \Omega_H M_H - \widetilde{M}_O\Esp_g[\Omega_{TOH}]\\
&=- \Omega_H M_H - \widetilde{M}_O P_{gOH} \odot \Omega_{OH}\\
\frac{\partial}{\partial S_H}&= -\frac{1}{2} \Omega_H\\
\frac{\partial}{\partial \widetilde{\beta}} &= \text{ moche ?}
\end{align*}
...
\begin{align*}
\partial_{S_H} (\Esp_h[\log h(Z_H\mid Z_O)]) &= \sum_i\partial_{S_{Hi}} (\Esp_h[\log h(Z_H\mid Z_O)])  \\
&=
\end{align*}
\subsection{E step}

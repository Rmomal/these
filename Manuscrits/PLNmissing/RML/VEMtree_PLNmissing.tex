...
\begin{empheq}[box=\lesswidefbox]{align*}\partial_{\widetilde{\beta}_{jk}} (\widetilde{B})& = [\bm{M}(\bm{W})]_{jk} \times \widetilde{B} 
\end{empheq}
Hence
...
\partial_{S_H} (\Esp_h[\log h(Z_H\mid Z_O)])&=\sum_i \frac{1}{2} (S_{Hi}^{-1})
\end{empheq}
...
Because $\Omega_{Tm}$ is tree-structured, its determinent factorizes on the edges of $T$. More precisely we obtain:
\begin{align*}
|\Omega_{Tm}| &= \prod_{k\in O} \omega_{kk} \prod_{kl \in T} \frac{\omega_{kk}\omega_{ll}-\omega_{kl}^2}{\omega_{kk}\omega_{ll}}\\
\log |\Omega_{Tm}|&= \sum_{k\in O} \log \omega_{kk} + \sum _{kl} \mathds{1}\{kl \in T\} \left[\log(\omega_{kk}\omega_{ll} - \omega_{kl}^2) - \log (\omega_{kk}\omega_{ll})\right]\\
\Esp_g[\log |\Omega_{Tm}|]&= \sum _{kl} P_{gkl} \left[ \log(\omega_{kk}\omega_{ll} - \omega_{kl}^2) - \log (\omega_{kk}\omega_{ll})\right]
\end{align*}

Finally we obtain:
\begin{align*}
 \Esp_{gh_O^*}[\log p(Z_O|T)] &= \frac{n}{2} \left(\sum _{kl} P_{gkl} \left[ \log(\omega_{kk}\omega_{ll} - \omega_{kl}^2) - \log (\omega_{kk}\omega_{ll})\right]\right) - \frac{1}{2}< \Esp_g[\Omega_{Tm}], \widetilde{M}_O^T\widetilde{M}_O + \widetilde{S}_O> 
\end{align*}

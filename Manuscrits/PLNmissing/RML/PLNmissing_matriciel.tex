...
Rewriting of $\Omega_T$, as a tree-structured filter on a matrix $\Omega$. Denoting $I_T = [I_{Tjk} = \mathds{1}\{jk \in T\}]_{jk}$:
$$ \Omega_T \equiv \left\{ \begin{array}{rl}
\omega_{Tjk} &= \omega_{jk} I_{Tjk}\\
\omega_{Tjj} &= \omega_{jj}
\end{array} \right. $$ 
We also have $ \Esp[I_T \mid Y] = P_T =[P_{Tjk}]_{jk}= [\mathds{P}\{jk \in T \mid Y \}]_{jk}$.\\

So we have $$\Esp\left[ Z^T \Omega_T Z \,\middle\vert\,  Y\right] = \Esp\left[\Esp\left[ Tr(Z^T Z \times(\Omega \odot I_T)) \,\middle\vert\,  Z_O,T\right] \,\middle\vert\,  Y\right]$$
\subsection{Surrogate quantity}

For $n$ samples:
\begin{align*}
Q&= \sum_{i=1}^{n} \Esp[\log p(Z,T) | Y_i ] \\
&= \sum_i  \Esp[\log p(T)+\log p(Z_i|T)|Y_i] \\
  &=\sum_i \Esp\left[\sum_{jk \in T} \log \beta_{jk} - \log B + \frac{1}{2} \log|\Omega_T| - \frac{1}{2} Z_i^T \Omega_T Z_i\,\middle\vert\,  Y_i\right]\\
  &=n \;\sum_{j<k} \Esp\left[\mathds{1}\{jk\in T\} \log \beta_{jk}  \,\middle\vert\,  Y\right]- \log B +\frac{n}{2}  \Esp\left[\log|\Omega_T| \,\middle\vert\,  Y\right] - \frac{1}{2} \sum_i\Esp\left[ Z_i^T \Omega_T Z_i \,\middle\vert\,  Y_i\right]
\end{align*} 

Denoting $\mathds{P}\{jk\in T \mid Y\}=P_{Tjk}$ we obtain:
\begin{empheq}[box=\lesswidefbox]{align*} 
  Q  &=  \sum_{j<k} P_{Tjk}\left( n \log \beta_{jk} + \frac{n}{2}\log\left( 1- \frac{\omega_{Tjk} ^2}{\omega_{Tjj}\omega_{Tkk}}\right)\right) -\log B  +\frac{n}{2}\sum_j \log(\omega_{Tjj})\\
  & \;\;\;\; - \frac{1}{2} \sum_i\Esp\left[\Esp\left[ Tr(Z_i^T Z_i\Omega_T ) \,\middle\vert\,  Z_{iO},T\right] \,\middle\vert\,  Y_i\right]
\end{empheq}

\section{EM algorithm: M step}
$$  \partial_{\Omega_T}Q = \frac{n}{2}\sum_{j<k}P_{jk}\; \partial_{\Omega_T}\left[\log\left(1-\frac{\omega_{Tjk}^2}{\omega_{Tj}\omega_{Tk}}\right)\right]+\frac{n}{2}\sum_j \partial_{\Omega_T} \left[\log(\omega_{Tj})\right] -\frac{1}{2}\sum_i\Esp\left[\Esp\left[\partial_{\Omega_T} Tr(Z_i^TZ_i\Omega_T) \,\middle\vert\, Z_O,T\right] \,\middle\vert\, Y\right]$$ \\


Derivation per block of square form terms:\\
\begin{align*}
\Esp[ \partial_{\Omega_T}Tr(Z^TZ\Omega_T) \mid Z_O,T] &= \Esp\left[ \partial_{\Omega}Tr(Z^T Z \times(\Omega \odot I_T)) \,\middle\vert\,  Z_O,T\right]\\
&=\Esp[Z^TZ\times I_T\mid Z_O,T]\\
&= I_T \times

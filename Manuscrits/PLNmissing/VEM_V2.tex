...
\paragraph{Trace of expectations\\}
 As $S$ is diagonal:
\begin{align*}
Tr(\Esp_g[\hat{\Omega}_T] (M^\intercal M +S)] &= \sum_{kl} \bar{\omega}_{kl} [M^\intercal M]_{kl}+ \sum_k  [M^\intercal M+S]_{kk} \widehat{\omega}_{Tkk}
\end{align*}

Finally, 
\begin{empheq}[box=\lesswidefbox]{align*}
\Esp_{gh} [\log p(\Ubf \mid T)] =& \frac{1}{2}\sum_{kl} \Esp_g[I_{Tkl}(\widehat{\omega}_{Tkl} [M^\intercal M]_{kl}- \frac{n}{2} \log|\widehat{R}_{Tkl}|)] -\frac{1}{2} \sum_k  [M^\intercal M+S]_{kk} \widehat{\omega}_{Tkk}- \frac{n(p+r)}{2}\log 2\pi
 \end{empheq}

\subsubsection{Details of $\Esp_g[\log g(T) - \log p(T)]$}
\begin{align*}
\log g(T) - \log p(T) &= \left(  \sum_{k<l}I_{Tkl}\log \widetilde{\beta}_{kl} - \log \widetilde{B}\right) - \left(  \sum_{k<l} I_{Tkl}\log {\beta}_{kl} - \log {B}\right)\\
&=\frac{1}{2}\sum_{kl} I_{Tkl}\log \frac{\widetilde{\beta}_{kl}}{{\beta}_{kl}} - \log \frac{\widetilde{B}}{B}
\end{align*}
$$\boxed{
\Esp_g[\log g(T) - \log p(T)] = \frac{1}{2}\sum_{jk} P_{kl}  \big(\log \widetilde{\beta}_{kl}-\log(\beta_{kl})\big) - \log \frac{ \widetilde{B}}{B} }$$
...
%\end{align*}
\begin{align*}
\entr[h(\Ubf_H)]&= \sum_i \entr[h(\Ubf_{Hi})] \\
&=-\frac{1}{2} \sum_i\log |S_{Hi}| - \frac{nr}{2} \log(2e\pi)
\end{align*}

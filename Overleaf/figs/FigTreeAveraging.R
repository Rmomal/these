...
for (t in 1:T.nb){
   ...
   if(export){pdf(paste0(FigDir, 'FigTreeAveraging-p', p, '-tree', t, '-seed', seed, '.pdf'))}
   # F_PlotWeightedGraph(tree.edge.width*T.prob[t]*T.list[[t]], xy)
   F_PlotWeightedGraph(.5*T.list[[t]], xy)
   if(export){dev.off()}
}
if(export){pdf(paste0(FigDir, 'FigTreeAveraging-p', p, '-avgtree-seed', seed, '.pdf'))}
F_PlotWeightedGraph(Pedge, xy)
if(export){dev.off()}

if(export){pdf(paste0(FigDir, 'FigTreeAveraging-p', p, '-graph-seed', seed, '.pdf'))}
